...
class MemoryFactory:
    ...
    @staticmethod
    async def create(
        provider_name: str = "local", config: Optional[Dict] = None
    ) -> BaseMemoryProvider:
        """
        ...
        if provider_name == "local":
            ...
            file_path = (
                config.get("file_path", "ai_worker_local_memory.json")
                if config
                else "ai_worker_local_memory.json"
            )
            provider = LocalJSONProvider(file_path=file_path)

        elif provider_name == "memu":
            # Direct import - let it fail if missing so we know
            ...
            print(
                f"Warning: Unknown provider '{provider_name}', falling back to local."
            )
